...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut Scanner<R>, out: &mut W) {
    let mut is_prime = vec![true;10_usize.pow(6) + 1];
    is_prime[0] = false;
    is_prime[1] = false;
    for i in 2..=10_usize.pow(6) {
        if is_prime[i] {
            for j in ((i+i)..=10_usize.pow(6)).step_by(i) {
                is_prime[j] = false;
            }
        }
    }
    ...
    for _ in 0..n {
        let i = scan.token::<f64>();
        let sr = i.sqrt();
        if sr - sr.floor() == 0.0 && is_prime[sr as usize] {
            writeln!(out, "YES");
        } else {
            writeln!(out, "NO");
        }
