...
fn solve<R: io::BufRead, W: io::Write>(scan: &mut Scanner<R>, out: &mut W) {
    let (y, k, n) = (scan.token::<usize>(), scan.token::<usize>(), scan.token::<usize>());

    let mut x = vec![];
    for tot in (k..=10_usize.pow(9)).step_by(k) {
        if tot > n {
            break;
        }
        if let Some(other) = tot.checked_sub(y) {
            if (other > 0) {
                x.push(other);
            }
        }
    ...
    if x.len() == 0 {
        ...
        writeln!(out, "{}", x.iter().map(|i| i.to_string()).collect::<Vec<String>>().join(" "));
    }
}
